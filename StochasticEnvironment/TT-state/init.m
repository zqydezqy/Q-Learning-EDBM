...
%      acc_res=StoDavis(Davis,v_forward(1,i));  %基本阻力产生的加速度   
    if(i<index_acc)%加速阶段
...
for i=size:-1:index_brake%计算后向速度和加速度，一直制动就好
    acc_res=(Davis(1)+Davis(2)*v_backward(1,i)+Davis(3)*(v_backward(1,i))^2)/Mass;
%     acc_res=StoDavis(Davis,v_backward(1,i));  %基本阻力产生的加速度   
    acc_backward(1,i)=-acc_res + MaxDeccel- acc_grad(1,i);%反向加速度
    v_backward(1,i-1)=(v_backward(1,i).^2+2*acc_backward(1,i)*(ds)).^0.5;
    if(v_backward(1,i-1)>vc)
        v_backward(1,i-1)=vc;
    end
end
